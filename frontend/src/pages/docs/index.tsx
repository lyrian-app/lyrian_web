      ...
      <div className={style.docs}>
        <Main>
          <div className={content.content}>
            ...
            <h3>ToC</h3>
            <ul>
              <li><a href="#Lyrianとは？">Lyrianとは？</a></li>
              <ul>
                <li><a href="#特徴">特徴</a></li>
              </ul>
              <li><a href="#使い方">使い方</a></li>
              <ul>
                <li><a href="#1_モデル作成">1. モデル作成</a></li>
                <li><a href="#2_単語の修正">2. 単語の修正</a></li>
                <li><a href="#3_歌詞の作成">3. 歌詞の作成</a></li>
                <li><a href="#4_完成">4. 完成</a></li>
              </ul>
            </ul>

            <h2 id="Lyrianとは？">Lyrianとは？</h2>
            <p>
              Lyrianとは、<span className={content.emphasis}>マルコフ連鎖を用いた日本語の歌詞生成ツール</span>です。ユーザーが予め入力した文章の中から単語をランダムに選択し、歌詞生成を行います。
            </p>
            ...
            <h4>1. ランダム性</h4>
            <p>
              Lyrianでは、<span className={content.emphasis}>マルコフ連鎖</span>を用いて歌詞生成を行います。マルコフ連鎖とは、ざっくりと説明すると「次の状態が、今現在の状態にのみ依存するようなモデル」のことを言います。
            </p>
            <p>
            ...
            <p>
              このように、<u>過去の状態に依存することなく、現在の状態のみによって、次の状態が決まるようなモデル</u>をマルコフ連鎖と呼びます。
            </p>
            <p>
              Lyrianでは、上記のような確率テーブルをユーザーが入力した文章から作成し、そのテーブルを基に歌詞生成を行います。つまり、単語を1つランダムに選択し、その単語の次に来得る単語を上記のような確率テーブルの基でランダムに決定し、そしてその単語の次に来得る単語を……といった具合に、<b>単語同士を確率的に繋げていくことで歌詞を生成</b>します。
            </p>
            <p>
              こうして生成される歌詞は、文章の文脈やコロケーションなどを無視して生成されるため、<span className={content.emphasis}>普段では思いつかないユニークな歌詞</span>を多く生成することができます。
            </p>

            <h4>2. リズミカル</h4>
            <p>
              日本語で歌詞を作成する場合、歌ったときのリズムが平坦になりやすいという特徴があります。それは、日本語が<span className={content.emphasis}>モーラ拍リズム</span>の言語であることに起因します。
            </p>
            <p>
            ...
            <p>
              一方で、英語などの言語は<span className={content.emphasis}>強勢拍リズム</span>という言語の分類に該当します。こちらは、「音節主音＋音節副音を単語の区切りの一単位とする言語」のことです（ここでは「音節主音＝基本的に母音」、「音節副音＝音節主音の前後にくっつく子音」程度に捉えてください）。
              <br />
            ...
            <p>
              このモーラ拍リズムと強勢拍リズムが作詞にどう影響するのかというと、<b>音符への言葉の割り当て方</b>に違いが出てきます。
            </p>
            <p>
              日本語はモーラ拍リズムの言語であるため、作詞を行うと1つの音符に対して1モーラしか割り当てられない箇所が必然的に多くなり、<u>歌ったときのリズムが平坦になりやすい</u>という特徴があります。
            </p>
            <p>
              一方、英語の場合は音節主音＋音節副音を一単位として歌詞を割り振るため、日本語の歌詞と比べたときに1つの音符あたりの音の情報量が多くなり、<u>楽譜には表記されない、言語自体が持つリズミカルさが音楽にも反映される</u>という特徴があります。
            </p>
            <p>
              これらの違いはあくまで「特徴」であり、言語自体の「良し悪し」では決してないのですが、英語圏の音楽の影響を強く受けた楽曲を作る際に、こうした言語の違いが障壁となるのもまた事実です。
            </p>
            <p>
              こうした問題を踏まえ、Lyrianでは音符に言葉を割り当てる際に、その言葉のシラブルや無声子音などを考慮することで、<span className={content.emphasis}>日本語でもリズミカルな歌詞が生成できる</span>よう工夫しています。
            </p>
            <p>
            ...
            <p>
              つまり「音楽」という単語は、モーラ単位では4つの音符が必要となりますが、<u>場合によっては2つの音符にも詰め込むことができる</u>、というわけです。
            </p>
            <p>
              このようにLyrianには、日本語でリズミカルな歌詞が制作するために、<b>1つの音符に対するサウンドの情報量を増やす</b>工夫が施されています。
            </p>
            ...
            <p>
              ユーザーが入力した文章から、モデルを作成します。モデルには<u>文章を品詞単位で分解した単語</u>と、<u>その単語が他のどの単語に繋がりうるかを計算した確率テーブル</u>が記録されています。
            </p>
            <p>
              <b>「編集中のデータをお持ちの方はこちら」</b>というボタンを押すことで、歌詞作成の工程でダウンロードした編集データをインポートすることができ、途中の状態から編集を再開することができます。
            </p>
            ...
            <p>
              Lyrianを用いた歌詞作成のコツとして、<span className={content.emphasis}>「更新ボタンを押しまくる」</span>と<span className={content.emphasis}>「アイデアのきっかけとして用いる」</span>という2つがあります。
            </p>
            <p>
              1つ目は単純に、直感的に良いと感じるフレーズが出てくるまで、とにかく更新ボタンを押しまくろう、というものです。
              <br />
              このアプリケーションはいわば<b>「作詞ガチャ」</b>なので、必ずしも1発目でSSR級のフレーズが生まれるわけではありません。なので、少しでもイマイチだと感じた歌詞は遠慮なく切り捨て、よりビビッと来るものを探し求めたほうが結果的に良い歌詞が生まれやすくなります。
            </p>
            <p>
